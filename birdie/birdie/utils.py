...
from birdie.common import lockutils
from birdie.common import processutils
...
synchronized = lockutils.synchronized_with_prefix('birdie-')
...
def find_config(config_path):
    ...
    :raises: `birdie.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(CONF.state_path, "etc", "birdie", config_path),
        os.path.join(CONF.state_path, "etc", config_path),
        os.path.join(CONF.state_path, config_path),
        "/etc/birdie/%s" % config_path,
    ]
...
def cinderdir():
    import birdie
    return os.path.abspath(birdie.__file__).split('birdie/__init__.py')[0]
...
def monkey_patch():
    ...
    Example: 'birdie.api.ec2.cloud:' \
     birdie.openstack.common.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See birdie.openstack.common.notifier.api.notify_decorator)
...
def get_root_helper():
    return 'sudo birdie-rootwrap %s' % CONF.rootwrap_config


def brick_get_connector_properties(multipath=False, enforce_multipath=False):
    """wrapper for the brick calls to automatically set
    the root_helper needed for birdie.
